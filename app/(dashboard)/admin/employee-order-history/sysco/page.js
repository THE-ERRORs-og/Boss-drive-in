...
export default function SyscoOrderHistory() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setIsLoading(true);
      try {
        const result = await getOrderHistoryByType("sysco", {
          page: currentPage,
        ...
        if (result.status === "SUCCESS") {
          ...
          toast({
            variant: "destructive",
            title: "Error",
            description: result.error || "Failed to fetch Sysco order history",
          });
        }
      } catch (error) {
  ...
  return (
    <div className="p-8 h-screen w-screen flex flex-col items-center">
      <h1 className="text-3xl font-semibold mb-4">Sysco Order History</h1>
