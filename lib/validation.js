...
export const userSchema = object({
  ...
  role: zEnum(["superadmin", "admin", "employee"], { required_error: "Role is required" }),
...
export const cashSummarySchema = object({
  ...
  ownedToRestaurantSafe: number({
    required_error: "OWED To Restaurant Safe is required",
  }),
  removalAmount: number()
    .min(0, "Removal Amount must be at least 0.")
    .optional(),
  removalItemCount: number()
    .int("Removal Item Count must be a whole number")
    .min(0, "Removal Item Count must be at least 0.")
    .optional(),
  discounts: number()
    .min(0, "Discounts must be at least 0.")
    .optional(),
