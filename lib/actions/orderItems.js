...
export const getOrderItems = async (orderType) => {
  try {
    await connectDB();

    const items = await OrderItem.find({ type: orderType, isEnabled: true })
      .sort({ order: 1 })
      .lean();
...
export const getAllOrderItems = async (orderType) => {
  ...
  try {
    ...
    const orderItems = await OrderItem.find({ type: orderType })
      .sort({ order: 1 })
      .lean();
...
export const createOrderItem = async (name, orderType, stockNo) => {
  const session = await auth();
  if (
    !session ||
    !session.user ||
  ...
  try {
    ...
    const existingItem = await OrderItem.findOne({ stockNo });
    if (existingItem) {
      return {
        status: "ERROR",
    ...
    const lastItem = await OrderItem.findOne({ type: orderType }).sort({
      order: -1,
    });
...
export const updateOrderItem = async (id, name, stockNo) => {
  ...
  try {
    ...
    // Check if stock number already exists for a different item
