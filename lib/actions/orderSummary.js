...
export const createOrderSummary = async (form) => {
  ...
  const { date, shiftTime, submissionDate, items } = form;
  ...
  try {
    const orderSummary = {
      ...
      items: items.map((item) => ({
        ...
        boh: parseFloat(item.boh.toFixed(2)), // Round to two decimal places
        cashOrder: parseFloat(item.cashOrder.toFixed(2)), // Round to two decimal places
        inventory: parseFloat(item.inventory.toFixed(2)), // Round to two decimal places
      })),
    };
