...
export const createSyscoOrder = async (orderData) => {
  ...
  try {
    ...
    // Parse the incoming string and create a Date object set to NOON UTC
    const dbDate = new Date(orderData.date + "T12:00:00Z");
    ...
    const emailData = {
      ...
      templateData: {
        ...
        dateStr: formatDate(newOrder.date),
        ordertype: "Sysco",
    ...
    const syscoEmailData = {
      templateName: "order-details-sysco",
      groupName: "Sysco Specific",
      templateData: {
        logosrc: process.env.LOGO_SRC || "",
        username: session.user.name,
        shiftTime: timeOptions[orderData.shiftNumber - 1],
    ...
    const emailPromises = [
      ...
      sendGroupEmail(syscoEmailData).catch((error) => {
    ...
    const [groupEmailResponse, syscoEmailResponse] = await Promise.all(
      emailPromises
    );
