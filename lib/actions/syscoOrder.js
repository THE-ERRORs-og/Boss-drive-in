...
export const createSyscoOrder = async (orderData) => {
  ...
  try {
    ...
    let groupEmailResponse;
    try {
      groupEmailResponse = await sendGroupEmail(emailData);
    } catch (error) {
    ...
    // Send to specific email address using the new sysco-specific route
    let syscoEmailResponse;
    try {
      const specificEmail =
        process.env.SYSCO_SPECIFIC_EMAIL || "sysco@example.com";
      syscoEmailResponse = await sendSyscoOrderEmail({
        to: specificEmail,
