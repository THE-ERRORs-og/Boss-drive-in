...
export const getOrderHistoryByType = async (type, params) => {
  ...
  try {
    ...
    const normalizeToObjectIds = (ids = []) =>
      ids
        .filter(Boolean)
        .map((id) =>
    ...
    const isAllLocations =
      !location || location === "__all__" || location.toLowerCase?.() === "all";

    if (isAllLocations) {
      // If user has access to all locations, don't filter by location
      if (!session.user?.hasAllLocationsAccess) {
        const accessibleIds = normalizeToObjectIds(
          session.user?.locationIds || []
        );
        locationFilter = accessibleIds.length
          ? { location: { $in: accessibleIds } }
          : { location: { $exists: false } };
      }
    } else if (mongoose.Types.ObjectId.isValid(location)) {
      locationFilter = { location: new mongoose.Types.ObjectId(location) };
    } else {
      // Invalid location value provided; fall back to accessible locations
      if (!session.user?.hasAllLocationsAccess) {
        const accessibleIds = normalizeToObjectIds(
          session.user?.locationIds || []
        );
...
export const getOrderHistory = async (params) => {
  ...
  try {
    ...
    // Get total counts
    const [usChefCount, restaurantDepotCount, specialOnlineCount, syscoCount] =
      await Promise.all([
    ...
    const allOrders = [
      ...usChefOrders.map((order) =>
        serializeOrder({ ...order, type: "USChef" })
      ),
      ...restaurantDepotOrders.map((order) =>
        serializeOrder({ ...order, type: "RestaurantDepot" })
      ),
      ...specialOnlineOrders.map((order) =>
        serializeOrder({ ...order, type: "SpecialOnline" })
      ),
      ...syscoOrders.map((order) =>
        serializeOrder({ ...order, type: "Sysco" })
      ),
    ];
    ...
    const totalRecords =
      usChefCount + restaurantDepotCount + specialOnlineCount + syscoCount;
