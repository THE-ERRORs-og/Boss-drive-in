...
export const depositSafeBalance = async (locationId) => {
  ...
  try {
    ...
    try {
      ...
      const currentSafeBalance = await Constant.findOne(query).populate(
        "location",
        "name"
      );
      ...
      if (!currentSafeBalance) {
        throw new Error(
          "Unable to fetch current safe balance for this location"
        );
      }
...
export const getCurrentSafeBalance = async (locationId) => {
  ...
  try {
    ...
    const currentBalance = await Constant.findOne(query)
...
export async function getSafeBalanceHistory({
  ...
  try {
    ...
    // Handle location filtering properly
    if (locationId && locationId !== "__all__") {
      // Only apply specific location filter if a valid locationId is provided
      conditions.location = locationId;
    } else if (!session.user.hasAllLocationsAccess) {
      // If user doesn't have all locations access, limit to their allowed locations
      conditions.location = { $in: session.user.locationIds };
    }
    // If locationId is "__all__" and user has all locations access, don't add any location filter
...
export async function addSafeBalance(
  amount,
  type,
  description,
  userId,
