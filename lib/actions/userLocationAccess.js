...
export async function addUserLocationAccess(userId, locationIds) {
  ...
  try {
    ...
    // For regular employees, ensure they can only have one location at a time
    if (user.role === "employee" && locationIds.length > 1) {
      return parseServerActionResponse({
        error: "Regular users can only have access to one location at a time",
        status: "ERROR",
      });
    }
    ...
    // For employees, replace existing location access instead of adding
    let updatedUser;
    if (user.role === "employee") {
      updatedUser = await User.findByIdAndUpdate(
        userId,
        { $set: { locationAccess: locationObjectIds } },
        { new: true }
      )
    ...
    return {
      status: "SUCCESS",
      message: user.role === "employee" ? "Location access updated successfully" : "Location access added successfully",
      data: {
        ...updatedUser,
